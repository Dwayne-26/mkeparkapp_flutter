...
class PredictTimeBar extends StatefulWidget {
  ...
  @override State<PredictTimeBar> createState()=>_PredictTimeBarState();
}
class _PredictTimeBarState extends State<PredictTimeBar>{
  DateTime _when = DateTime.now();
  Future<void> _pickDate() async {
    final d = await showDatePicker(context: context, initialDate: _when, firstDate: DateTime.now().subtract(const Duration(days:1)), lastDate: DateTime.now().add(const Duration(days:30)));
